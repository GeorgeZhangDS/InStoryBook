...
3. **ImageGenerationAgent** - 图片生成Agent（×4，并行执行）
   - 使用Planner生成的image_description直接生成图片
   - 调用图片生成API
   - 处理图片base64数据
   - 与Writer并行执行（不依赖Writer完成）

4. **QualityCheckAgent** - 质量检查Agent
   - 内容安全性检查
   - 质量评估

5. **FormatterAgent** - 格式化Agent
   - 格式化最终输出
   - 生成JSON结构
...
START → PlannerAgent (输入验证)
    │
    ├─ 输入不完整 → 返回needs_info=True → END (返回文字给用户，等待补充)
    │
    └─ 输入完整 → 生成故事大纲（包含image_description）
                              │
                ┌─────────────┴─────────────┐
                │                           │
        FanOut(4个WriterAgents并行)  FanOut(4个ImageGenAgents并行)
                │                           │
                │                           │
                └─────────────┬─────────────┘
                              ↓
...
[Planner Agent] - 输入验证阶段
    ...
    └─ 输入完整? ──是──→ 继续执行
                    ...
                    [Planner Agent] - 规划阶段 (1-2秒)
                          │
                          ▼
                    生成故事大纲（内存中，包含image_description）
    │
    ├─────────────────────────────────────┐
    │                                     │
    ├──────┬──────┬──────┐    ┌──────┬──────┬──────┬──────┐
    ▼      ▼      ▼      ▼    ▼      ▼      ▼      ▼      ▼
[Writer1][Writer2][Writer3][Writer4]  [ImgGen1][ImgGen2][ImgGen3][ImgGen4]
    │      │      │      │    │      │      │      │      │
    │      │      │      │    │      │      │      │      │
    │      │      │      │    │      │      │      │      │
    │      │      │      │    │      │      │      │      │
